                ...
                <div class="accordion-content">
                    <div class="images-media-grid-create">
                        ...
                        <div class="main-image-panel-create">
                            ...
                            <input type="file" name="image" id="main-image-create" accept="image/*" class="hidden">
        ...
        if (form) {
            form.addEventListener('submit', function(e) {
                ...
                if (!categorySelect || !categorySelect.value) {
                    e.preventDefault();
                    if (categorySelect) {
                        const accordionContent = categorySelect.closest('.accordion-content');
                        if (accordionContent && !accordionContent.classList.contains('open')) {
                            toggleAccordion(accordionContent.previousElementSibling);
                        }
                ...
                if (!descTextarea || !descTextarea.value.trim()) {
                    e.preventDefault();
                    if (descTextarea) {
                        const accordionContent = descTextarea.closest('.accordion-content');
                        if (accordionContent && !accordionContent.classList.contains('open')) {
                            toggleAccordion(accordionContent.previousElementSibling);
                        }
                        alert('Veuillez renseigner la description complète.');
                        descTextarea.focus();
                    }
                    return;
                }

                // Validate Image (required on creation)
                const imageInput = document.getElementById('main-image-create');
                if (!imageInput || !imageInput.files || imageInput.files.length === 0) {
                    e.preventDefault();
                    if (imageInput) {
                        const accordionContent = imageInput.closest('.accordion-content');
                        if (accordionContent && !accordionContent.classList.contains('open')) {
                            toggleAccordion(accordionContent.previousElementSibling);
                        }
                ...
                // Validate Price
                const priceInput = document.getElementById('regular-price');
                if (!priceInput || !priceInput.value || parseFloat(priceInput.value) < 0) {
                    e.preventDefault();
                    if (priceInput) {
                        const accordionContent = priceInput.closest('.accordion-content');
                        if (accordionContent && !accordionContent.classList.contains('open')) {
                            toggleAccordion(accordionContent.previousElementSibling);
                        }
                        alert('Veuillez renseigner un prix valide (supérieur ou égal à 0).');
                        priceInput.focus();
                    }
                    return;
                }
                ...
                if (!stockInput || !stockInput.value || parseInt(stockInput.value) < 0) {
                    e.preventDefault();
                    if (stockInput) {
                        const accordionContent = stockInput.closest('.accordion-content');
                        if (accordionContent && !accordionContent.classList.contains('open')) {
                            toggleAccordion(accordionContent.previousElementSibling);
                        }
                        alert('Veuillez renseigner la quantité en stock (supérieure ou égale à 0).');
                        stockInput.focus();
                    }
                    return;
                }
