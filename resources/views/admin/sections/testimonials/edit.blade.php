...
<div class="space-y-6">
    ...
    <!-- Form -->
    <form action="{{ route('admin.sections.testimonials.update', $testimonial) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-6">
            ...
            @if($testimonial->avatar)
                <div>
                    ...
                    <div class="border-2 border-gray-200 rounded-lg p-4 bg-gray-50 text-center">
                        ...
                        @endphp
                        <img src="{{ $avatarUrl }}" alt="{{ $testimonial->name }}" class="w-32 h-32 mx-auto rounded-full shadow-md object-cover">
                    </div>
                </div>
            @endif
            ...
            <div>
                ...
                <input type="text" id="name" name="name" value="{{ old('name', $testimonial->name) }}" maxlength="255" required class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-[#003e87] focus:border-transparent" placeholder="e.g., Sophie Martin">
                @error('name')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="text" id="role" name="role" value="{{ old('role', $testimonial->role) }}" maxlength="255" class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-[#003e87] focus:border-transparent" placeholder="e.g., Client vérifié">
                @error('role')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea id="content" name="content" rows="6" maxlength="1000" required class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-[#003e87] focus:border-transparent resize-none" placeholder="Write the customer's review here...">{{ old('content', $testimonial->content) }}</textarea>
                <p class="text-xs text-gray-500 mt-1">
                    <span id="content-count">{{ strlen(old('content', $testimonial->content)) }}</span>/1000 characters
                </p>
                @error('content')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="number" id="order" name="order" value="{{ old('order', $testimonial->order) }}" min="0" required class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-[#003e87] focus:border-transparent" placeholder="0">
            ...
            <div class="flex items-center">
                ...
                <input type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $testimonial->is_active) ? 'checked' : '' }} class="w-5 h-5 text-[#003e87] border-gray-300 rounded focus:ring-[#003e87]">
